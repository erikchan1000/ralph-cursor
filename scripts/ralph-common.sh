...
Before doing anything, use the Read tool to open these files (DO NOT use shell commands like cat):
1. Read \`RALPH_TASK.md\` (task and completion criteria)
...
Use tools for all I/O:
- Use the Read tool to open files (e.g., \`Read(path: "frontend/src/app/page.tsx")\`).
- Use the Edit tool to modify files; do not print file contents back unless necessary.
- Use the Shell tool for commands; prefer \`rg\` over \`grep\` when searching.
- Do NOT inline huge file contents into your responses; keep outputs concise.
- After changes, run tests or build via Shell as appropriate.
...
run_iteration() {
  ...
  # Determine agent binary (env override > autodetect > fallback error)
  local agent_bin="${RALPH_AGENT_BIN:-}"
  if [[ -z "$agent_bin" ]]; then
    if command -v cursor-agent >/dev/null 2>&1; then
      ...
      echo "❌ No agent binary found (expected 'cursor-agent' or 'agent')." >&2
      echo "   Install cursor-agent: curl https://cursor.com/install -fsS | bash" >&2
      echo "   Or set RALPH_AGENT_BIN to the correct binary." >&2
      echo ""
      echo "GUTTER" > "$fifo"
      return
    fi
  elif ! command -v "$agent_bin" >/dev/null 2>&1; then
    echo "❌ RALPH_AGENT_BIN='$agent_bin' not found on PATH." >&2
    if command -v cursor-agent >/dev/null 2>&1; then
      agent_bin="cursor-agent"
      echo "   Falling back to: $agent_bin" >&2
    elif command -v agent >/dev/null 2>&1; then
      agent_bin="agent"
      echo "   Falling back to: $agent_bin" >&2
    else
      echo "   No compatible agent found." >&2
      echo "GUTTER" > "$fifo"
      return
    fi
  fi
  ...
  # Build cursor agent command parts
  local -a agent_args
  agent_args=(-p --force --output-format stream-json --model "$MODEL")
  ...
  (
    # Send Ralph prompt to the agent via stdin
    printf "%s\n" "$prompt" | "$agent_bin" "${agent_args[@]}" 2>&1 | "$script_dir/stream-parser.sh" "$workspace" > "$fifo"
  ) &
  local agent_pid=$!
